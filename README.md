...
- **30 MCP tools in 6,810 tokens.** That's the measured size of the full `tools/list` response (o200k tokenizer). Your entire tracker costs about as much context as one long file read.
...
- **Read long pages a section at a time.** A page over 30,000 characters comes back as an outline with section sizes plus its opening, and `get_page(section="Current state")` fetches just that part. Nothing is silently cut off by the harness.
