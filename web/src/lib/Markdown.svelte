<script lang="ts">
  ...
  let html = $derived(
    DOMPurify.sanitize(decorateAttachmentLinks(linkIdentifiers(rendered)), {
      // Keep the data-mermaid / data-lang / data-issue-ident hooks the
      // post-render effects read, plus data-attachment (chip decoration) and
      // the download attr on attachment chips.
      ADD_ATTR: [
        "data-mermaid",
        "data-lang",
        "data-issue-ident",
