...
test("offers a resize for an image past the long-edge threshold", () => {
  const offer = decideDownscale(
    facts({ width: 6000, height: 4000, bytes: 2 * MB }),
  );
...
test("offers a resize once the file passes 80% of the cap", () => {
  ...
  const under = decideDownscale(
    facts({ width: 3000, height: 2000, bytes: 7 * MB }),
    cap,
  );
  expect(under).toBeNull();

  const over = decideDownscale(
    facts({ width: 3000, height: 2000, bytes: 9 * MB }),
    cap,
  );
...
test("declines to offer when resizing would change nothing", () => {
  ...
  const offer = decideDownscale(
    facts({ width: 2000, height: 1200, bytes: 9 * MB }),
  );
...
test("leaves ordinary images alone", () => {
  expect(
    decideDownscale(facts({ width: 1920, height: 1080, bytes: 400_000 })),
  ).toBeNull();
  expect(
    decideDownscale(facts({ width: 3200, height: 1800, bytes: 2 * MB })),
  ).toBeNull();
});
...
test("ignores images with no measurable dimensions", () => {
  expect(
    decideDownscale(facts({ width: 0, height: 0, bytes: 9 * MB })),
  ).toBeNull();
});
...
test("scaled dimensions keep the aspect ratio and never upscale", () => {
  expect(scaledDimensions(6000, 4000, 2560)).toEqual({
    width: 2560,
    height: 1707,
  });
  expect(scaledDimensions(4000, 6000, 2560)).toEqual({
    width: 1707,
    height: 2560,
  });
...
test("reads the byte cap out of the server's rejection", () => {
  expect(parseUploadCap("file too large: 12345678 bytes (max 10485760)")).toBe(
    10485760,
  );
  expect(parseUploadCap("Payload too large, maximum 5242880 bytes")).toBe(
    5242880,
  );
