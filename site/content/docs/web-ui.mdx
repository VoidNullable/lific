...
The **Waiting on** field in the issue sidebar holds blockers that are not issues: a person, or a window of days with a note. Select **Add blocker**, choose **Person** or **Dates**, and add an optional note. Each blocker shows whether it is still waiting, due to check, or overdue, and its clear button removes it. List rows and board cards carry a small indicator naming who or when the issue waits on; overdue waits are shown in red. See [waiting on a person or a date](/docs/concepts/issues#waiting-on-a-person-or-a-date).
...
For a project already loaded in the browser, the palette searches its local issue and page index first. Sparse results trigger server full-text search, whose matches appear below the local results. When no server result contains every word, the group is labelled "partial matches". Context menus and other overlays take ownership of their shortcuts while open.
