...
A stdio MCP session has no transport authentication, so it carries its identity in the `LIFIC_TOKEN` environment variable instead. `lific connect --stdio` writes that variable into the client's configuration entry. Lific resolves it once at startup and uses it for the whole session.
