...
`lific mcp --instances <FILE>` runs a stdio MCP proxy in front of several
separately authenticated Lific servers. The agent gets one MCP entry, and every
tool call names the server it is for.

The proxy routes **tool calls only**. It answers `initialize`, `ping` and
`tools/list` itself, and refuses `resources/*`, `prompts/*`, `completion/*` and
`logging/setLevel` with `-32601`. Lific servers advertise no resources or
prompts, so nothing a Lific server offers is lost. If you also point it at
something that does serve them, give that server its own MCP entry.

Use it when you keep a private tracker and also work in a community tracker.
Instead of two copies of every tool, the agent sees one `create_issue` with an
`instance` argument. The servers remain separate databases; this connection
does not synchronize them. Publishing a project does not grant anonymous access
to its MCP endpoint.

The proxy makes independent authenticated requests to each server. Servers need
no shared configuration, and each alias's credential is sent only to that
alias's configured URL.

For a single server, see [MCP](/docs/mcp) or the
...
An alias is at most 32 characters of lowercase letters, digits, `-` and `_`,
...
A call the proxy cannot route fails with JSON-RPC error `-32602` rather than a
tool result, so a client can tell "never sent" apart from a refusal by the
tracker, which arrives as `Error:` text as usual.
...
`get_issue`, `get_board`, `get_briefing`, `list_resources`, `get_page`,
`get_plan`, `get_activity`, `list_comments`, `list_attachments`,
`get_attachment`, `export`. It is not derived from the backend's `readOnlyHint`, because that is
...
because two instances can legitimately hold the same identifier: `APP-42` on
`private` and `APP-42` on `community` are different issues, and the payloads may
...
- **No resources or prompts.** `resources/*`, `prompts/*`, `completion/*` and
  ...
  meaningless behind one connection.
