...
| `blocked` | `boolean` | No | Only return issues with at least one unresolved blocker (inverse of workable). Each result lists its blockers as blocked_by:LIF-3,LIF-7, and any waits as waiting_on:@user or waiting_until:DATE. |
...
Each wait on the issue gets its own line, for example `Waiting on @blake (decide the schema)`, `Waiting until 2026-09-28..29 (state filing office)`, `Due to check since 2026-09-28, expected by 2026-09-29` or `Overdue since 2026-09-30, expected 2026-09-28..29`. `list_issues` and `get_board` rows carry the compact form: `waiting_on:@blake`, `waiting_until:2026-09-28..29`, `due_since:2026-09-28` or `overdue_since:2026-09-30`.
...
Issue relations in the frontmatter (`blocks`, `blocked_by`, `relates_to`, `duplicates`, `duplicated_by`) leave out issues in projects you cannot view, with no placeholder or count.
...
Links two issues with a relation. A `blocks` link can instead make `target` wait on a person or on a window of days: omit `source` and pass either `user`, or `from` with an optional `until`. See [waiting on a person or a date](/docs/concepts/issues#waiting-on-a-person-or-a-date).

| Name | Type | Required | Description |
| --- | --- | --- | --- |
| `source` | `string` | No | Source issue identifier (e.g. PRO-1). Required unless this is a wait. |
| `target` | `string` | Yes | Target issue identifier (e.g. PRO-2). For a wait, the issue that waits. |
| `relation_type` | `string` | Yes | Relation type: blocks, relates_to, or duplicate. A wait must use blocks. |
| `user` | `string` | No | Username the target waits on until the wait is cleared |
| `from` | `string` | No | Date (YYYY-MM-DD) the target waits for. It blocks before this day. |
| `until` | `string` | No | Last expected day of a `from` wait. After it the wait is overdue. Defaults to `from`. |
| `note` | `string` | No | Note for a user or date wait |

`link_issues(target="LIF-9", relation_type="blocks", from="2026-09-28", until="2026-10-02", note="state filing office")` makes LIF-9 wait for that window. An unknown username, an `until` before `from`, or a wait the issue already has is an error.

### `unlink_issues`

Removes a relation between two issues, or clears a wait on `target`.

| Name | Type | Required | Description |
| --- | --- | --- | --- |
| `source` | `string` | No | First issue identifier. Omit to clear a wait. |
| `target` | `string` | Yes | Second issue identifier, or the issue that waits |
| `user` | `string` | No | Clear the target's wait on this username |
| `from` | `string` | No | Clear the target's date waits that start on this day (YYYY-MM-DD) |
...
Gets a page and its content. A page of up to 30,000 characters is returned whole. A longer page returns its outline and the start of its content. Any response cut at the budget ends with the exact call that continues it (`offset`) and the number of characters left. It also suggests reading by section when a heading starts in the unread part. Cuts fall at the end of a line when one ends within the last 2,000 characters. The budget keeps a response under the roughly 50 KB at which agent harnesses cut tool output off.
...
| `since_seq` | `integer` | No | A `seq` from an earlier read. Returns only what changed in the content since then, as a unified diff with two lines of context, plus the current `seq`. Cannot be combined with `section`, `outline` or `offset`. |
| `offset` | `integer` | No | Character offset at which to continue a cut read: into the page, or into the section when `section` is given. Returns a line giving the position, then up to 30,000 characters, with no outline. An offset at or past the end is an error. Cannot be combined with `outline`. |
...
The response opens with the current time and a `since` value to pass next time. That cursor is one second before the briefing, because `since` is strict and timestamps have one-second resolution: a change written later in the briefing's final second is reported by the next briefing instead of being skipped, and changes from that second may be reported twice. The sections that follow are built from live records when the call is made. Nothing is stored or summarized by a model.
...
- **Active plans**, with progress and the next open step. The next step is the first unfinished step in tree order, descending into its first unfinished child. When that step mirrors an issue in a project you cannot view, the issue is not named.
- **Due to check**: open issues with a date wait whose window has arrived, most overdue first, each with its wait line (`Overdue since 2026-09-30, expected 2026-09-28..29 (note)`). These no longer block, so this is where the follow-up surfaces.
- **Blocked** issues that are still open, each with the issues that block it and their status, plus any wait still holding it (`waiting on @blake (note)`, `waiting until 2026-09-28..29`). A blocker in a project you cannot view is counted but not named.
...
Sections with nothing to report are left out. Issue sections show up to 8 items, plans up to 5, and each kind of change up to 5. The whole response stays under 6,000 characters. When a section is cut, either by its own limit or to fit that budget, it ends with the number of items left out and the call that lists them all. Sections count up to 100 items; a count such as `100+` means there are at least that many.
...
Audit timestamps are stored in UTC with one-second resolution, and `since` is read on the same clock. A bare date such as `2026-06-01` means midnight UTC. A datetime without an offset is UTC, and one with `Z` or an offset such as `-05:00` is converted to UTC. Fractional seconds are ignored. Entries in the same second as the cursor are excluded. To resume after the last entry you read without missing anything written later in its second, pass a cursor one second earlier than its timestamp and expect that second's entries to repeat.
