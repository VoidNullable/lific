...
Defaults do not create a database. Ordinary local commands refuse a missing database and name the expected path. Run `lific init` first, restore a backup, or use the explicit [container bootstrap](/docs/self-hosting#containers). `lific doctor` can diagnose an absent or unreadable config; it skips checks that depend on that config rather than silently using defaults.
...
A trusted proxy can instead pass the client address directly in `X-Lific-Client-IP`, authenticated by `X-Lific-Proxy-Secret`. Lific accepts that pair only from a peer in `trusted_proxies`, and only when the secret matches the `LIFIC_TRUSTED_PROXY_SECRET` environment variable. If a trusted peer sends either header without a valid pair, rate-limited routes answer `503` rather than falling back to a shared bucket. Set the variable before the proxy starts sending the headers, use a long random value, and have the proxy remove any client-supplied copies of both headers.
...
| Key | Type | Default | Description |
| --- | --- | --- | --- |
| `trash_days` | unsigned integer | `30` | Days a deleted issue, page or comment stays in the trash, where it can still be restored, before it is removed permanently. `0` never empties the trash. |

The server sweeps the trash 30 seconds after it starts and then every six hours, so an item can outlive its window by up to six hours. The sweep runs only while `lific start` is serving. Changing the window takes effect at the next restart.
