...
`lific connect` configures AI coding tools to use your Lific server over MCP. It detects the clients installed on your machine and writes a `lific` server entry into each client's MCP configuration, in one of three forms: local stdio, remote HTTP with an API key, or remote HTTP with OAuth. It knows 11 clients, and can write the OAuth form for 8 of them.

For stdio and remote, `lific connect` mints each client its own credential unless you pass `--key` to reuse one. With `--oauth` it mints nothing, and the client signs in itself.
...
If your database lives only on a remote server, use the
[remote stdio proxy](/docs/repository-bindings#use-a-remote-server) instead of
running `connect` without a local instance. To let an agent infer its project
from a checkout, set up a [repository binding](/docs/repository-bindings).
[Multi-instance MCP](/docs/mcp/multi-instance) routes one agent connection to
several servers with separate credentials and explicit write destinations.

## Supported clients

| Client | Id | Global configuration | Project configuration | `--oauth` | Stdio carries `LIFIC_TOKEN` |
| --- | --- | --- | --- | --- | --- |
| OpenCode | `opencode` | `~/.config/opencode/opencode.json` | `./opencode.json` | Yes | Yes |
| Claude Code | `claude-code` | `~/.claude.json` | `./.mcp.json` | Yes | Yes |
| Claude Desktop | `claude-desktop` | per OS, see below | none | No | No |
| Cursor | `cursor` | `~/.cursor/mcp.json` | `./.cursor/mcp.json` | Yes | No |
| VS Code | `vscode` | per OS, see below | `./.vscode/mcp.json` | Yes | No |
| Codex | `codex` | `~/.codex/config.toml` | `./.codex/config.toml` | Yes | Yes |
| Zed | `zed` | `~/.config/zed/settings.json` | none | Yes | No |
| Gemini CLI | `gemini` | `~/.gemini/settings.json` | `./.gemini/settings.json` | Yes | No |
| Windsurf | `windsurf` | `~/.codeium/windsurf/mcp_config.json` | none | Yes | No |
| Goose | `goose` | `~/.config/goose/config.yaml` | none | No | No |
| Crush | `crush` | `~/.config/crush/crush.json` | `./crush.json` | No | No |

With `--oauth`, Claude Desktop, Goose and Crush are skipped with a note, and the rest of the run continues. Claude Desktop's OAuth path is its Settings > Connectors screen, not a configuration file, so add Lific there. Goose and Crush have no confirmed OAuth support; connect them with the default API key mode.

The last column matters for `--stdio`. A client whose MCP schema has no environment field gets the launch command alone, and that session runs as the operator rather than as its own agent.
...
In stdio mode, the entry runs `lific --db <absolute-db-path> mcp` and carries the minted token in the client's environment field as `LIFIC_TOKEN`. The field name follows the client: `environment` for OpenCode, `env` for Claude Code and Codex. The other clients get the launch command alone, as the table above notes.
...
Re-running `lific connect` is safe and idempotent. It replaces the `lific` entry with current values, rotates that client's credential, and repairs an entry that points at a stale URL or is missing its token. Reconnecting reuses the same bot for that owner and client, even if the owner has been renamed.
...
When `--key` is not supplied, `lific connect` mints one credential per selected client in both remote and stdio mode. The plaintext value is embedded in the written configuration and is not otherwise persisted by connect. On a server with human accounts, the minted key belongs to a bot user named `{tool}-{owner}`, so the connected tool acts as itself and inherits that owner's project access. Re-running rotates the existing active key for the same tool. Disconnecting or deleting a bot revokes its OAuth tokens as well.

The owner is the only human account, or the only administrator when there are several humans. Otherwise pass `--user <username>`. Without it, a remote run fails, and a stdio run warns and writes an entry with no `LIFIC_TOKEN`, which runs as the operator until you reconnect with `--user`. On a server with no human accounts yet, each client gets a plain key named after its id, bound to no user.
...
- `LIFIC_TOKEN` is the identity of a local stdio MCP session. `lific connect --stdio` writes it into the client's environment field. The CLI may also use it for remote access when the target origin matches `LIFIC_URL`; it is not an unrestricted credential for every URL. Multi-instance MCP reads it only if an alias explicitly names it with `token_env`.
