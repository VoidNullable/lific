...
Without `--here` or `--config`, `lific init` uses operating system configuration and data directories. On Linux, it writes the configuration file to `~/.config/lific/lific.toml` and the database to `~/.local/share/lific/lific.db`.
