...
The REST default is `asc` for compatibility, so the default REST page of a long thread is its _oldest_ 50 comments and says nothing about the newest one. Ask for `order=desc` to read the current end of a conversation. The MCP tool, the CLI, and the web UI all default to newest first for exactly that reason.
