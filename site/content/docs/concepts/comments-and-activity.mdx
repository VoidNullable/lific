...
A comment's kind is `comment`, or `verification` when it records the evidence an agent gave while closing an issue. Verification comments are marked `[verification]` in MCP output and carry a Verification badge in the web UI. Otherwise they behave like any other comment: the author can edit or delete them.
