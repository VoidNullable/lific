...
Agents read pages through `get_page`, and agent harnesses silently cut off tool output past roughly 50 KB. A page longer than 30,000 characters is therefore returned as its outline, with each section's size, plus the start of its content. The agent then reads the part it needs with `get_page(identifier="LIF-DOC-3", section="Current state")`. The web UI always shows the whole page. Creating or editing a page past that size through MCP adds a note to the response suggesting a split, or moving old history to an archive page.
