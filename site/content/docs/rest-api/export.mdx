...
Issue and project exports are scoped to the caller. An issue's frontmatter lists its relations (`blocks`, `blocked_by`, `relates_to`, `duplicates` and `duplicated_by`), and issues in projects the caller cannot read are left out of those lists with no placeholder or count, so an export does not reveal that they exist. Page exports carry no relations. [`lific export`](/docs/cli) with the local backend reads the database file directly and is not filtered.
