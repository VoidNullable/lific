...
Files may be at most 10 MiB; a larger one is `400`. Lific decides the type from the file's own bytes, not from the name or the type the client declares, and accepts PNG, JPEG, GIF, WebP, SVG, PDF, plain text, ZIP, MP4, WebM video and audio, Ogg audio, MP3, SQLite databases and legacy HWP. HWP shares its file signature with other formats, so it is accepted only when the filename ends in `.hwp`; a request that declares an HWP type for any other name is `400`. HWP is always served as a download. An empty file, an unlisted type or `entity_type` without `entity_id` is `400`.
