...
Rows on both endpoints are _skinny_: no issue `description`, no page `content`, no comment body. Fetch those from the entity's own endpoint when a detail view needs them.
