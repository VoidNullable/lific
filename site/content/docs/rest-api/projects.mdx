...
- `comment`: `issue_id`, `page_id`, `user_id`, `username`, `created_at`, `updated_at`, `comment_kind` (`comment` or `verification`; named so because `kind` is the change discriminator).
