...
Lists an issue's waits: blockers that are a person or a window of days rather than another issue. Path parameter: `id` (integer issue ID). It has no query parameters or request body. Requires read access to the project.

Each wait has `id`, `issue_id`, `kind` (`user` or `date`), `note`, `state` and `created_at`. A user wait adds `user_id`, `username` and `display_name`. A date wait adds `earliest` and `latest` (`YYYY-MM-DD`). `state` is `holding` (blocks), `due` (from `earliest` through `latest`) or `overdue` (after `latest`), computed on the server's local day when the response was built. Issue responses and the sync rows from `/index` and `/changes` carry the same objects under `waits`.

### POST `/api/issues/{id}/waits`

Adds a wait. Path parameter: `id` (integer issue ID). Requires write access to the project.

Request body fields are `user` (string username), or `from` (string date) with optional `until` (string date, defaults to `from`), plus optional `note` (string, up to 500 characters). Pass `user` or `from`, not both. An unknown or deactivated username and an `until` before `from` return 400; a wait the issue already has returns 409. Response: the new wait.

### DELETE `/api/issues/{id}/waits/{wait_id}`

Clears a wait. Path parameters are `id` (integer issue ID) and `wait_id` (integer). Requires write access to the project. A `wait_id` that belongs to another issue returns 404. Response: `{ "cleared": true }`.

Adding or clearing a wait publishes `issue.updated` for the issue.

### GET `/api/clock`

Returns the server's calendar day and UTC offset, which decide whether a date wait is holding, due or overdue. It has no parameters or request body. Requires a signed-in caller. Response: `{ "today": "2026-09-26", "utc_offset_minutes": -300 }`, where the offset is minutes east of UTC. A client that evaluates wait states itself should compute today at this offset rather than in its own timezone.
...
No request body. Response: a `Comment` array, capped by row count and a 2 MiB byte budget. A short page does not necessarily end the thread. Read `x-comment-has-more` and `x-comment-returned`; continuation headers are `x-comment-next-offset` in offset mode and `x-comment-next-created-at` plus `x-comment-next-id` for descending reads.

If the first comment cannot fit, the server returns `413` with an explicit offset or cursor you can use to deliberately skip it. Comments are never silently skipped.

### POST `/api/issues/{issue_id}/comments`

Creates a comment on an issue. Path parameter: `issue_id` (integer). It has no query parameters.

Request body field: `content` (string). Response: a `Comment` object. After normalization, new bodies cannot exceed 256 KiB. Each body allows at most 256 distinct mention tokens and 256 distinct attachment IDs. Exceeding either limit returns `400`.
...
Request body field: `content` (string). Response: a `Comment` object. The create limits apply, except an existing body above 256 KiB may be edited without growing. Reference limits have no exception: reduce excess references before saving. Rejected edits leave the comment unchanged. The HTTP request body limit remains 2 MiB.
...
Issue and project exports are scoped to the caller. The `blocks`, `blocked_by`, `relates_to`, `duplicates` and `duplicated_by` frontmatter lists leave out issues in projects the caller cannot view, with no placeholder or count, so an export does not reveal that those issues exist. Page exports render no relations. The local CLI export (`lific export`, direct SQL) reads the database file and is not filtered.
...
In `fts` mode every word must match. If nothing in the requested scope contains every word, the endpoint returns the hits that contain any of them instead, ranked by relevance, and each of those objects carries `partial_match: true`. The field is absent from ordinary results.
