...
Both local stdio and the remote proxy can supply an omitted project when the current checkout is [bound to a project](/docs/repository-bindings). The server retains 31 tools; multi-instance mode adds the proxy's `list_instances` tool and an `instance` selector to the forwarded tools.
