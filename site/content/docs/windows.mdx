...
Native source builds require Rust 1.88 or later, the Visual Studio C++ build
tools, and [Bun](https://bun.sh). Build the web UI first so it is embedded in
the binary:
...
The published Windows artifact uses the same MSVC target, cross-built on Linux
through the repository's `release-windows-msvc` Devenv profile. CI runs that
executable on Windows and checks its checksum, startup, database, API, and
embedded frontend before publication. Native Windows Clippy and Rust tests
remain a separate CI check.
