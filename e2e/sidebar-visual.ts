...
export async function sidebarContrast(page: Page, selector = "aside") {
  return page.locator(selector).evaluate((root) => {
    ...
    const selected = root.querySelector<HTMLElement>(
      '.sidebar-destination[aria-current="page"]',
    );
    ...
    return {
      ...
      selectedText: selected
        ? ratio(rgb(getComputedStyle(selected).color), background(selected))
        : null,
      avatarText: avatar
        ? ratio(rgb(getComputedStyle(avatar).color), background(avatar))
        : null,
...
export async function checkSidebarContrast(page: Page, directory: string) {
  ...
  const selected = page.locator(
    'aside #project-nav-1 > a[aria-current="page"]',
  );
  ...
  for (const theme of ["light", "dark"])
    for (const accent of accents) {
      ...
      assert.ok(
        result.marker >= 3 && result.focusOnSelection >= 3,
        JSON.stringify(result),
      );
      assert.notEqual(
        await selected.evaluate((el) => getComputedStyle(el).backgroundColor),
        await neutral.evaluate((el) => getComputedStyle(el).backgroundColor),
        "Selection differs from hover",
      );
      assert.equal(
        await page
          .locator('aside [data-sidebar-project="1"]')
          .evaluate(
            (el) => getComputedStyle(el.parentElement!).backgroundColor,
          ),
        "rgba(0, 0, 0, 0)",
        "Expanded active parent stays transparent",
      );
      results.push(result);
    }
  await Bun.write(
    resolve(directory, "sidebar-contrast.json"),
    JSON.stringify(results, null, 2),
  );
  console.table(
    results.map(
      ({ theme, accent, minimumText, selectedText, avatarText, marker }) => ({
        theme,
        accent,
        minimumText: minimumText.toFixed(2),
        selectedText: selectedText?.toFixed(2),
        avatarText: avatarText?.toFixed(2),
        marker: marker.toFixed(2),
      }),
    ),
  );
}
...
export async function loadVisualFonts(page: Page) {
  await page.addStyleTag({
    url: "https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&family=DM+Sans:ital,opsz,wght@0,9..40,300..700;1,9..40,300..700&display=swap",
  });
  await page.evaluate(async () => {
    await Promise.all([
      document.fonts.load('500 13px "DM Sans"'),
      document.fonts.load('500 18px "Space Grotesk"'),
    ]);
    if (
      !document.fonts.check('500 13px "DM Sans"') ||
      !document.fonts.check('500 18px "Space Grotesk"')
    )
      throw new Error("Visual fonts did not load");
  });
}
      ...
      for (const scale of ["sm", "md", "lg"])
        for (const compact of [false, true]) {
          ...
          await page.screenshot({
            path: resolve(
              directory,
              `desktop-${theme}-${width}-${scale}-${compact ? "compact" : "comfortable"}.png`,
            ),
          });
          assert.ok(
            await page
            ...
            "Sidebar must not scroll horizontally",
          );
        }
  ...
  for (const theme of ["light", "dark"]) {
    ...
    for (const position of ["top", "bottom"]) {
      ...
      await page.screenshot({
        path: resolve(
          directory,
          `desktop-multiple-expanded-${theme}-${position}.png`,
        ),
      });
    }
  }
...
async function touchGeometry(page: Page, selector: string) {
    ...
    [...root.querySelectorAll<HTMLElement>("a, button")]
      ...
      .flatMap((el) => {
        ...
        const siblings = [...el.parentElement!.children].filter(
          (sibling) => sibling !== el && sibling.matches("a, button"),
        );
        const overlap = siblings.some((sibling) => {
          const other = sibling.getBoundingClientRect();
          return (
            Math.min(b.right, other.right) - Math.max(b.left, other.left) > 1 &&
  ...
  assert.deepEqual(
    failures,
    [],
    "Every mobile control retains a non-overlapping, in-bounds 44px touch target",
  );
  const mainRows = page.locator(
    `${selector} .mobile-project-row, ${selector} nav .sidebar-destination`,
  );
  assert.ok(
    await mainRows.evaluateAll((rows) =>
      rows.every((el) => el.getBoundingClientRect().height >= 48),
    ),
    "Main navigation keeps 48px or larger rows at every text size",
  );
  assert.ok(
    await page
      .locator(`${selector} nav`)
      .evaluate((el) => el.scrollWidth <= el.clientWidth),
    "No horizontal mobile navigation scroll",
  );
}

export async function mobileVisualChecks(
  page: Page,
  directory: string,
  screenshots: boolean,
) {
  ...
  for (const theme of ["light", "dark"])
    for (const accent of accents) {
      ...
      assert.ok(
        rootResult.minimumText >= 4.5,
        JSON.stringify({ theme, accent, rootResult }),
      );
      await root
        .getByRole("button", { name: "Open One navigation", exact: true })
        .click();
      await depth(2);
      const projectResult = await sidebarContrast(
        page,
        "[data-mobile-project]",
      );
      assert.ok(
        projectResult.minimumText >= 4.5 && projectResult.focusOnSelection >= 3,
        JSON.stringify({ theme, accent, projectResult }),
      );
  ...
  await Bun.write(
    resolve(directory, "mobile-sidebar-contrast.json"),
    JSON.stringify(results, null, 2),
  );
  for (const theme of ["light", "dark"])
    for (const scale of ["sm", "md", "lg"])
      for (const compact of [false, true]) {
        ...
        if (screenshots) {
          ...
          await page.screenshot({
            path: resolve(directory, `mobile-root-${suffix}.png`),
          });
        }
        await root
          .getByRole("button", { name: "Open One navigation", exact: true })
        ...
        if (screenshots) {
          ...
          await page.screenshot({
            path: resolve(directory, `mobile-project-${suffix}.png`),
          });
        }
        await page.keyboard.press("Escape");
        await depth(1);
