...
const watchdog = setTimeout(() => {
  void finish(1, "Archive browser smoke exceeded 180 seconds");
}, 180_000);
let finishing = false;
...
async function instance(name: string, seed: boolean) {
  ...
  const cli = (args: string[]) =>
    execFileSync(BIN, ["--config", config, "--db", db, ...args], {
      encoding: "utf8",
  ...
  if (seed) {
    ...
    cli([
      ...
      "Archive graph root",
      "--description",
      "Missing source file: /api/attachments/999999",
      "--json",
    ]);
    cli([
      "issue",
    ...
    cli([
      ...
      "ARC-1",
      "--content",
      "A comment carried across instances",
      "--json",
    ]);
    cli([
      "page",
      ...
      "Archive notebook",
      "--content",
      "A page carried across instances",
      "--json",
    ]);
  }
  const sql = new Database(db);
  const admin = sql
    .query("SELECT username FROM users WHERE is_admin=1 AND is_bot=0")
  ...
  const login = async (identity: string) => {
    const res = await fetch(`${base}/api/auth/login`, {
      ...
      body: JSON.stringify({ identity, password: PASSWORD }),
    });
  ...
  const api = async (path: string, method = "GET", body?: unknown) => {
    ...
    assert(
      ...
      `${method} ${path} did not return JSON. Rebuild the backend with the archive routes before running this test.`,
    );
    return res.json();
  };
...
function watch(page: Page) {
  ...
  page.on("console", (message) => {
    ...
    if (
      /^Failed to load resource:.*(400|403|409|422|net::ERR_FAILED)/.test(
        message.text(),
      )
    )
      return;
...
async function staleDownload(
  ...
  const panel = page.locator("section").filter({
    has: page.getByRole("heading", { name: "Project archive", exact: true }),
  });
  ...
  const download = (event: { suggestedFilename: () => string }) =>
    downloads.push(event.suggestedFilename());
  page.on("download", download);
  const aborted = page.waitForEvent("requestfailed", {
    predicate: (request) => request.url().endsWith("/api/project-archives/ARC"),
    timeout: 10_000,
  });
  ...
  assert.deepEqual(
    downloads,
    [],
    "stale archive never triggers a browser download",
  );
...
async function main() {
  ...
  assert.equal(
    caps.can_import,
    true,
    "fresh backend must expose archive capabilities",
  );
  assert.deepEqual(
    await b.api("/projects"),
    [],
    "destination starts with no projects",
  );
  await a.api("/issues/link", "POST", {
    source: "ARC-1",
    target: "ARC-2",
  ...
  form.append(
    "file",
    new File(["Archive attachment bytes"], "archive-note.txt", {
      type: "text/plain",
    }),
  );
  form.append("entity_type", "issue");
  form.append("entity_id", String(issue.id));
  const uploaded = await fetch(`${a.base}/api/attachments`, {
    method: "POST",
  ...
  const exportPanel = source.locator("section").filter({
    has: source.getByRole("heading", {
  ...
  assert(
    Bun.gunzipSync(await Bun.file(archivePath).bytes()).length > 0,
    "download is valid gzip",
  );
  ...
  await destination
    .getByRole("button", { name: "Import a project archive", exact: true })
    .click();
  await destination
    .getByLabel("Project archive (.tar.gz)", { exact: true })
    .setInputFiles(archivePath);
  await destination
    .getByRole("checkbox", { name: /I understand this imports/ })
    .check();
  let posts = 0;
  destination.on("request", (request) => {
    if (
      request.method() === "POST" &&
      request.url().endsWith("/api/project-archives")
    )
      posts++;
  });
  const importResponse = destination.waitForResponse(
    (response) =>
      response.request().method() === "POST" &&
      response.url().endsWith("/api/project-archives"),
  );
  await destination
  ...
  const totalRows = Object.values(
    result.report.rows as Record<string, number>,
  ...
  assert(
    destination.url().endsWith("/projects/import"),
  ...
  assert(
    ...
    "mobile report overflows",
  );
  await destination.screenshot({
    path: join(
      process.env.E2E_SCREENSHOT_DIR ?? tmpdir(),
      "lific-archive-import-mobile.png",
    ),
    fullPage: true,
  });
  ...
  await destination
    .getByText("archive-note.txt", { exact: true })
    .first()
    .click();
  await visible(destination, "Archive attachment bytes");
  const attachmentDownload = destination.waitForEvent("download");
  await destination
    .getByRole("button", { name: "Download archive-note.txt", exact: true })
    .click();
  const attachment = await attachmentDownload;
  assert.equal(
    await Bun.file((await attachment.path())!).text(),
    "Archive attachment bytes",
  );
  await destination.goto(`${b.base}/ARC/pages`);
  await destination
    .getByText("Archive notebook", { exact: true })
    .filter({ visible: true })
    .first()
    .click();
  ...
  console.log(
    "ok  empty-instance import, counts, other-tab discovery, graph, comment, page, attachment, privacy, mobile report",
  );
  ...
  await destination.evaluate((max) => {
    const file = new File(["size-probe"], "too-large.tar.gz", {
  ...
  console.log(
    "ok  duplicate ID, invalid extension, server-sourced browser size cap",
  );
  ...
  assert(
    await destination
      .getByRole("button", { name: "Import in progress" })
      .isDisabled(),
  );
  await destination
  ...
  console.log(
    "ok  in-flight navigation and duplicate-submit guard, lost response remains unknown after reload, no automatic retry",
  );
  ...
  deniedForm.append(
    "archive",
    new File([await Bun.file(archivePath).bytes()], "ARC.lific.tar.gz", {
      type: "application/gzip",
    }),
  );
  const denied = await fetch(`${b.base}/api/project-archives`, {
    method: "POST",
  ...
  assert.equal(
    denied.status,
    403,
    "non-admin archive POST is refused in legacy mode",
  );
  ...
  await staleDownload(source, a.base, () =>
    changeSession(source, a.base, sourceSecondToken),
  );
  const renewedSourceToken = await a.login("second-admin");
  await staleDownload(source, a.base, () =>
    changeSession(source, a.base, renewedSourceToken),
  );
  console.log(
    "ok  delayed export aborts on project navigation, account change and same-user session replacement, no stale download event",
  );
  ...
  const secondUser = await (
    await fetch(`${b.base}/api/auth/me`, {
  ...
  assert.equal(
    await destination
      .getByText("The import may have completed", { exact: false })
      .count(),
    0,
  );
  assert.equal(
    await destination.evaluate((key) => sessionStorage.getItem(key), firstKey),
    "1",
  );
  await destination
    .getByLabel("Project archive (.tar.gz)", { exact: true })
  ...
  await destination.route("**/api/project-archives", async (route) => {
    ...
    await route.fulfill({
      status: 201,
      json: {
        project: { id: 900, identifier: "PRIVATEOLD", is_public: false },
        report: {
          project: "PRIVATEOLD",
          rows: { issues: 1 },
          blobs: 0,
          external_references: ["private-old-account-reference"],
  ...
  assert.equal(
    await destination.getByText("PRIVATEOLD", { exact: false }).count(),
    0,
  );
  assert.equal(
    await destination
      .getByText("private-old-account-reference", { exact: false })
      .count(),
    0,
  );
  assert.equal(
    await destination.getByText("Import in progress", { exact: true }).count(),
    0,
  );
  await destination.unroute("**/api/project-archives");
  await destination
  ...
  assert.equal(
    await destination.evaluate((key) => sessionStorage.getItem(key), secondKey),
    "1",
  );
  await changeSession(destination, b.base, secondToken);
  await visible(destination, "The import may have completed");
  const refreshed = await fetch(`${b.base}/api/auth/me/refresh`, {
    method: "POST",
    headers: {
  ...
  await destination
    .getByText("Sign in to import a project archive.", { exact: true })
    .waitFor();
  assert.equal(
    await destination
      .getByText("The import may have completed", { exact: false })
  ...
  console.log(
    "ok  pending warnings isolated by user ID, old-account progress/report hidden, own-only reset, session-refresh and logout retention",
  );
  assert.equal(errors.length, 0, errors.join("\n"));
  console.log(
    "ok  regular-user controls hidden and POST denied in legacy mode, mobile export, no unexpected console errors",
  );
}
