...
const server = await createServer({
  ...
  plugins: [
    {
      name: "mobile-nav-fixture",
      enforce: "pre",
      resolveId(id) {
        if (id === "/src/MobileNavFixture.svelte") return fixtureId;
      },
      load(id) {
        ...
        server.middlewares.use(async (req, res, next) => {
          ...
          res.end(
            await server.transformIndexHtml(
              "/",
              `<div id="app"></div><script type="module">import {mount} from 'svelte'; import Fixture from '/src/MobileNavFixture.svelte'; import '/src/app.css'; mount(Fixture,{target:document.getElementById('app')});</script>`,
            ),
          );
        });
      },
...
const deadline = setTimeout(() => {
  console.error("Mobile navigation test deadline exceeded");
  process.exit(1);
}, 90_000);
let browser;
try {
  ...
  const depth = (n: number) =>
    page.waitForFunction((n) => history.state?.lificMobileNav?.depth === n, n);
  const active = () =>
    page.evaluate(
      () =>
  ...
  const closed = () =>
    page.waitForFunction(() =>
      document.querySelector("[data-mobile-nav]")?.hasAttribute("inert"),
    );
  ...
  assert.equal(
    await page.evaluate(
      () => (document.querySelector("#background") as HTMLElement).inert,
    ),
    true,
  );
  ...
  await depth(0);
  await closed();
  assert.deepEqual(await page.evaluate(() => (window as any).navCalls), [
    "/ONE/issues",
  ]);
  ...
  assert.deepEqual(await page.evaluate(() => (window as any).moves), [
    ["project", 1, "down"],
  ]);
  ...
  const input = rootPane.getByRole("textbox", {
    name: "Group name",
    exact: true,
  });
  ...
  assert.equal(await input.inputValue(), "Personal");
  assert.match(
    await rootPane.getByRole("alert").innerText(),
    /Name already exists/,
  );
  ...
  await input.waitFor({ state: "hidden" });
  await rootPane
    .getByRole("button", { name: "Actions for Personal", exact: true })
    .waitFor();
  ...
  await closed();
  assert.equal(
    await page.evaluate(() => (window as any).navCalls.includes("/cancelled")),
    false,
  );
  await openRoot();
  await rootPane
  ...
  await openRoot();
  const rootHistory = await page.evaluate(() => ({
    state: history.state,
  ...
  await rootPane
    .getByRole("button", { name: "Open One navigation", exact: true })
    .click();
  const projectHistory = await page.evaluate(() => ({
    state: history.state,
    length: history.length,
  }));
  ...
  await page.reload();
  await page.waitForFunction(
    () => typeof (window as any).finishLoading === "function",
  );
  assert.equal(await panel.count(), 0);
  await page.evaluate(() => (window as any).finishLoading());
  await projectPane
    .getByRole("heading", { name: "One", exact: true })
    .waitFor();
  assert.equal(await projectPane.getAttribute("inert"), null);
  assert.equal(await active(), "Projects");
  assert.deepEqual(
  ...
  await page.waitForFunction(
    () =>
      !!document.activeElement?.closest("[data-mobile-project]:not([inert])"),
  );
  assert.equal(await projectPane.getAttribute("inert"), null);
  assert.deepEqual(await page.evaluate(() => (window as any).navCalls), [
    "/another-route",
  ...
  await projectPane
    .getByRole("heading", { name: "Project unavailable", exact: true })
    .waitFor();
  ...
  await projectPane
    ...
    .click();
  await depth(1);
  const theme = rootPane.getByRole("button", {
    name: "Choose theme, current: system",
    exact: true,
  });
  assert.equal(await theme.getAttribute("aria-haspopup"), "menu");
  assert.equal(
    await theme.getAttribute("title"),
  ...
  assert.equal(
    await rootPane.getByRole("alert").innerText(),
    "Project order wasn't saved: Connection lost.",
  );
  ...
  for (const invalid of [
    ...
    { depth: "1" },
    { project: "ONE" },
    { depth: 2, project: "../outside" },
    { href: "https://elsewhere.invalid/" },
  ]) {
    await page.evaluate((invalid) => {
      history.replaceState(
        { lificMobileNav: { ...history.state.lificMobileNav, ...invalid } },
        "",
      );
    }, invalid);
    await page.reload();
    await page.waitForFunction(
      () => typeof (window as any).sharedOpen === "function",
    );
    assert.equal(
      await page.evaluate(() => (window as any).sharedOpen()),
      false,
    );
    await page.locator("#trigger").click();
    await depth(1);
  ...
  try {
    ...
    assert.equal(
      await fallback.evaluate(() => typeof crypto.randomUUID),
      "undefined",
    );
    const callsBefore = await fallback.evaluate(
      () => (window as any).randomValueCalls,
    );
    const fallbackDepth = (depth: number) =>
      fallback.waitForFunction((depth) => {
        ...
        const pane = panel.querySelector(
          depth === 1 ? "[data-mobile-root]" : "[data-mobile-project]",
        );
        return (
          !panel.hasAttribute("inert") && pane && !pane.hasAttribute("inert")
        );
      }, depth);
    await fallback.locator("#trigger").click();
    await fallbackDepth(1);
    assert.ok(
      (await fallback.evaluate(() => (window as any).randomValueCalls)) >
        callsBefore,
    );
    const session = await fallback.evaluate(
      () => history.state.lificMobileNav.session,
    );
    assert.match(
      session,
    ...
    await fallback.goForward();
    await fallbackDepth(1);
    const beforeReload = await fallback.evaluate(() => ({
      state: history.state,
      length: history.length,
    }));
    ...
    assert.equal(
      await fallback.evaluate(() => history.state.lificMobileNav.session),
      session,
    );
  ...
  console.log(
    "Mobile navigation passed: modal/focus, Back/Forward and rapid traversals, reload/async mount, LAN HTTP UUID fallback, missing projects, malformed history, route unwind/cancellation, links, menus/theme, move errors, group edit, breakpoint.",
  );
} finally {
  await browser?.close();
  await server.close();
