  ...
  plugins: [
    {
      ...
      enforce: "pre",
      resolveId(id) {
        if (id === "/src/SidebarFixture.svelte") return fixtureId;
      },
      load(id) {
        ...
        server.middlewares.use(async (req, res, next) => {
          ...
          res.end(
            await server.transformIndexHtml(
              "/",
              `<div id="app"></div><script type="module">import {mount} from 'svelte'; import Fixture from '/src/SidebarFixture.svelte'; import '/src/app.css'; mount(Fixture,{target:document.getElementById('app')});</script>`,
            ),
          );
        });
      },
...
const longName =
  "The extraordinarily long project name that must not hide its actions";
const longTitle =
  "Investigate the extraordinarily long sidebar regression title with enough detail to wrap across several lines";
const destinations = [
  "Overview",
  "Issues",
  "Board",
...
class API {
  ...
  fail(method: string, path: string) {
    ...
    await context.route("**/api/**", async (route) => {
      ...
      try {
        ...
        if (index >= 0) {
          ...
          if (gate.status !== 200) {
            ...
            return await route.fulfill({
              status: gate.status,
              json: { error: "Fixture rejected this save." },
            });
          }
        }
        if (method === "GET" && this.failingGets.has(path)) {
          ...
          return await route.fulfill({
            status: 409,
            json: { error: "Fixture read temporarily unavailable." },
          });
        }
        let data: unknown;
        if (path === "/auth/me" && ["GET", "PATCH"].includes(method)) {
          ...
          assert.deepEqual(
            [...body.ids].sort(),
            this.projects.map((p) => p.id).sort(),
          );
          this.projects = body.ids.map((id: number, sort_order: number) => ({
          ...
          assert.deepEqual(
            [...body.ids].sort(),
            this.groups.map((g) => g.id).sort(),
          );
          this.groups = body.ids.map((id: number, sort_order: number) => ({
            ...this.groups.find((g) => g.id === id)!,
            sort_order,
          }));
          data = this.groups;
          ...
          const group = this.groups.find(
            (g) => g.id === Number(path.split("/").at(-1)),
          )!;
          group.name = body.name;
          data = group;
        } else if (
          /^\/projects\/\d+\/my-role$/.test(path) &&
          method === "GET"
        ) {
          data = { role: "owner", enforced: true, is_admin: false };
        } else if (
          ["/issues", "/modules", "/pages", "/plans"].includes(path) &&
          method === "GET"
        ) {
          ...
          data =
            !project ||
            (path === "/pages" && url.searchParams.get("status") !== "active")
              ? []
              : [
                  {
                    id: projectId * 10,
        ...
        else if (["/auth/bots", "/folders"].includes(path) && method === "GET")
          data = [];
        else {
...
const deadline = setTimeout(() => {
  console.error("Sidebar suite deadline exceeded");
  process.exit(1);
}, 180_000);
...
async function session(
    ...
    await context.addInitScript((ids) => {
      ...
      if (localStorage.getItem("lific:sidebar:collapsed-groups") === null)
        localStorage.setItem(
          "lific:sidebar:collapsed-groups",
          JSON.stringify(ids),
        );
    }, options.collapsed);
  const errors: string[] = [];
  context.on("page", (page) => {
    ...
    page.on("console", (message) => {
      ...
      if (
        api.expectedConsole.has(message.location().url) &&
        /^Failed to load resource: the server responded with a status of 409\b/.test(
          message.text(),
        )
      )
        return;
  ...
  await page
    .locator('aside a[title="Account settings"]')
...
async function healthy(page: Page, api: API) {
  assert.equal(
    await page.locator("#boundary-error").count(),
    0,
    "Svelte boundary rendered",
  );
...
async function settle(s: Session) {
  ...
  assert.equal(
    s.api.calls.length,
    calls,
    "HTTP requests continued after the UI settled",
  );
      ...
      new Promise<void>((resolve, reject) => {
        ...
        function check() {
          if (el.getAttribute(args.name) === args.value) resolve();
            ...
            reject(
              new Error(
                `${args.name}: expected ${args.value}, got ${el.getAttribute(args.name)}`,
              ),
            );
  ...
  try {
    ...
    for (const s of sessions.slice(start)) {
      console.error("Diagnostics:", {
        errors: s.errors,
        unexpected: s.api.unexpected,
        requests: s.api.calls.length,
        boundary: await s.page
          .locator("#boundary-error")
      ...
      await s.page
        .screenshot({
          path: resolve(
            shotDir,
            `sidebar-failed-${name.replace(/[^a-z0-9]+/gi, "-")}.png`,
          ),
        })
        .catch(() => {});
    }
...
try {
  ...
  await test("separate disclosure and Overview links", async () => {
    ...
    const { page, aside } = s;
    await aside
      .getByRole("button", { name: "Expand One", exact: true })
      .click();
    assert.equal(new URL(page.url()).hash, "#/");
    await aside
      .getByRole("button", { name: "Expand Two", exact: true })
      .click();
    ...
    await aside
      .getByRole("button", { name: "Collapse One", exact: true })
      .click();
  ...
  await test("sidebar selection and readable accents in both themes", async () => {
    ...
    await s.aside
      .getByRole("button", { name: "Recent issues", exact: true })
      .click();
  ...
  await test("selected account metadata remains readable across accents", async () => {
    ...
    await Bun.write(
      resolve(shotDir, "sidebar-account-contrast.json"),
  ...
  await test("direct link reveal and deliberate collapse survives revalidation", async () => {
    ...
    assert.ok(
      await aside
        .getByRole("button", { name: "Current: One", exact: true })
        .isVisible(),
    );
    assert.ok(
      (
        await page.evaluate(() =>
          JSON.parse(localStorage.getItem("lific:sidebar:collapsed-groups")!),
        )
      ).includes(11),
    );
  ...
  await test("failed initial groups recover and reveal the active project exactly once", async () => {
    ...
    const { page, aside } = s;
    const scrollCalls = () =>
      page.evaluate(() => (window as any).sidebarScrollCalls);
    await settle(s);
    assert.ok(
      await aside.locator('a[title="One"]').isVisible(),
      "Projects loaded despite failed groups",
    );
    ...
    assert.deepEqual(
      await scrollCalls(),
      [],
      "Failed membership read must not consume route reveal",
    );
    assert.deepEqual(
      await page.evaluate(() =>
        JSON.parse(localStorage.getItem("lific:sidebar:collapsed-groups")!),
    ...
    assert.deepEqual(
      await scrollCalls(),
      [],
      "Wait for recovered membership, not just project success",
    );
    gate.release();
    await recovering;
    await attr(aside.locator("#group-11"), "hidden", null);
    ...
    await attr(aside.locator("#group-12"), "hidden", "");
    await settle(s);
    assert.deepEqual(await scrollCalls(), [
      { id: "1", options: { block: "nearest" } },
    ]);
    assert.deepEqual(
      await page.evaluate(() =>
        JSON.parse(localStorage.getItem("lific:sidebar:collapsed-groups")!),
    ...
    for (const path of ["/ONE/pages", "/ONE/board"]) {
      ...
      assert.deepEqual(
        await scrollCalls(),
        [{ id: "1", options: { block: "nearest" } }],
        "Revalidation must not reveal twice",
      );
    }
    assert.ok(
      await aside
  ...
  await test("later failed group snapshot defers newly visible project reveal until recovery", async () => {
    ...
    await settle(s);
    assert.ok(
      await aside.locator('a[title="Five"]').isVisible(),
      "New project arrived in the successful project response",
    );
    assert.equal(
      await group.locator('a[title="Five"]').count(),
      0,
      "Failed group response leaves the older membership snapshot in place",
    );
    await attr(group, "hidden", "");
    ...
    assert.deepEqual(
      await scrollCalls(),
      [],
      "An older successful group load must not permit premature reveal",
    );
    ...
    assert.deepEqual(
      await scrollCalls(),
      [],
      "Recovery must wait for the current membership response",
    );
    gate.release();
    await recovering;
    await settle(s);
    ...
    for (const collapsed of [false, true, false]) {
      if (collapsed) {
        await aside
          .getByRole("button", { name: "Collapse Five", exact: true })
          .click();
        await aside.getByRole("button", { name: "Work", exact: true }).click();
      } else if ((await group.getAttribute("hidden")) !== null) {
        await aside.getByRole("button", { name: "Work", exact: true }).click();
        await aside
          .getByRole("button", { name: "Expand Five", exact: true })
          .click();
      }
      ...
      assert.deepEqual(
        await scrollCalls(),
        revealed,
        "Later snapshots neither re-reveal nor override deliberate disclosure choices",
      );
    }
  });
  ...
  await test("long sidebar reveals nearest once and respects subsequent user scrolling", async () => {
    ...
    assert.ok(
      (await target.boundingBox())!.y > navBox.y + navBox.height,
      "Target begins below the visible sidebar",
    );
    ...
    assert.ok(
      targetBox.y >= navBox.y &&
        targetBox.y + targetBox.height <= navBox.y + navBox.height + 1,
      "Route entry scrolls target into view",
    );
    assert.ok(
      Math.abs(targetBox.y + targetBox.height - navBox.y - navBox.height) <= 2,
      "Nearest scroll aligns the offscreen row to the lower edge",
    );
    assert.deepEqual(
      await page.evaluate(() => (window as any).sidebarScrollCalls),
    ...
    await page.waitForFunction(
      () => document.querySelector("aside nav")!.scrollTop === 0,
    );
    for (const path of ["/P45/issues/P45-1", "/P45/pages"]) {
      ...
      await settle(s);
      assert.equal(
        await nav.evaluate((el) => el.scrollTop),
        0,
        "Within-project navigation must not fight manual scrolling",
      );
      assert.deepEqual(
        await page.evaluate(() => (window as any).sidebarScrollCalls),
        [{ id: "45", options: { block: "nearest" } }],
    ...
    assert.ok(
      ...
      "Entering another project reveals it again",
    );
    assert.deepEqual(
      await page.evaluate(() => (window as any).sidebarScrollCalls),
      [
  ...
  await test("stable destinations and cached recents across slow failed refreshes", async () => {
    ...
    assert.deepEqual(
      await nav
        .locator(":scope > a")
        .allTextContents()
        .then((rows) => rows.map((s) => s.trim())),
      destinations,
    );
    const boardY = (await nav
    ...
    for (const section of ["issues", "modules", "pages", "plans"]) {
      ...
      await routeTo(
        page,
        `/ONE/${section}/${section === "issues" ? "ONE-1" : "10"}`,
      );
    ...
    await attr(recent, "aria-busy", "true");
    assert.equal(
      await recent.locator("a").count(),
      0,
      "No previous-project cache leakage",
    );
    gate.release();
    await attr(recent, "aria-busy", "false");
  ...
  await test("long titles focus hover and 230 190 pixel widths", async () => {
    ...
    for (const width of [230, 190]) {
      const separator = aside.getByRole("separator", {
        name: "Resize sidebar",
      });
      ...
      await recent.focus();
      assert.equal(
        await recent.evaluate((el) => el.matches(":focus-visible")),
        true,
      );
      ...
      assert.ok(
        ...
        "Focus title stays inside sidebar",
      );
      assert.ok(
        await tip.evaluate(
          (el) =>
        ...
        "Focus title wraps without clipping",
      );
      ...
      assert.ok(
        actionBox.x + actionBox.width <= width,
        "Long project preserves visible actions",
      );
      assert.equal(
        await aside
          .locator("nav")
        ...
        true,
        "No horizontal sidebar scroll",
      );
    }
    assert.equal(
      await page.evaluate(() => localStorage.getItem("lific:sidebar:width")),
      "190",
    );
    await page.reload();
    await attr(
      aside.getByRole("separator", { name: "Resize sidebar" }),
      "aria-valuenow",
      "190",
    );
  });
  ...
  await test("text-scaled default preserves pixel preference and resize semantics", async () => {
    ...
    const stored = () =>
      page.evaluate(() => localStorage.getItem("lific:sidebar:width"));
    const width = async (expected: number) => {
      await attr(handle, "aria-valuenow", String(expected));
      assert.ok(
        Math.abs((await aside.boundingBox())!.width - expected) < 0.02,
        "ARIA reports the rendered CSS-pixel width",
      );
    };
    const scale = async (name: string) => {
    ...
    assert.equal(
      await stored(),
      null,
      "Text changes do not create a manual preference",
    );
    await handle.click();
    assert.equal(
      await stored(),
      null,
      "Clicking the resize handle is not a resize",
    );

    // Simulate a pre-upgrade preference, including a load already using Large.
    await scale("L");
    await page.evaluate(() =>
      localStorage.setItem("lific:sidebar:width", "300"),
    );
    await page.reload();
    await width(300);
    await scale("S");
    ...
    await page.evaluate(() =>
      localStorage.setItem("lific:sidebar:width", "190"),
    );
    await page.reload();
    await width(190);
    await scale("L");
    ...
    assert.equal(
      await stored(),
      "190",
      "Temporary minimum does not overwrite the saved width",
    );
    await page.reload();
    await width(202.5);
    await scale("M");
    ...
    assert.equal(
      await stored(),
      "212.5",
      "Keyboard resize is still ten physical pixels",
    );
    ...
    assert.equal(
      await stored(),
      "212.5",
      "Dragging does not persist until release",
    );
    await page.mouse.up();
    ...
    assert.equal(
      await stored(),
      null,
      "Reset resumes the proportional default",
    );
  ...
  await test("native modified sidebar links", async () => {
    ...
    await aside
      ...
      .click();
    const links = [
      aside.locator('a[title="Two"]'),
      aside.locator('#project-nav-1 > a[href="#/ONE/board"]'),
      aside.locator(".recent-link").first(),
      aside.locator('a[title="Account settings"]'),
    ];
  ...
  await test("group create rename failure retry Cancel Escape", async () => {
    const s = await session();
    const { page, aside, api } = s;
    const input = aside.getByRole("textbox", {
      name: "Group name",
      exact: true,
    });
    for (const mode of ["create", "rename"]) {
      ...
      for (const cancel of ["Cancel", "Escape"]) {
        ...
        const trigger = aside.getByRole("button", {
          name:
            mode === "create"
              ? "New project or group"
              : "Actions for group Work",
          exact: true,
        });
        assert.equal(
          await trigger.evaluate((el) => document.activeElement === el),
          true,
          "Cancel restores the edit trigger",
        );
      }
      await start();
      ...
      assert.match(
        await aside.getByRole("alert").innerText(),
        /Fixture rejected/,
      );
      ...
      await input.waitFor({ state: "hidden" });
      await aside
        .getByRole("button", { name: `Actions for group ${name}`, exact: true })
        .waitFor();
  ...
  await test("personal group and grouped project order payloads rollback", async () => {
    ...
    const groupOrder = () =>
      aside
        .locator('button[aria-label^="Actions for group "]')
        .evaluateAll((els) => els.map((el) => el.getAttribute("aria-label")));
    const projectOrder = () =>
      aside
        .locator("#group-11 a[title]")
        .evaluateAll((els) => els.map((el) => el.getAttribute("title")));
    const originalGroups = await groupOrder();
    for (const kind of ["group", "project"]) {
      const path =
        kind === "group" ? "/project-groups/reorder" : "/projects/reorder";
      const trigger =
        kind === "group" ? "Actions for group Work" : "Actions for One";
      ...
      assert.deepEqual((await called(api, 1, "PUT", path)).body, {
        ids: downIds,
      });
      await settle(s);
      assert.deepEqual(await readOrder(), [...original].reverse());
      await action(s, trigger, "Move up");
      assert.deepEqual((await called(api, 2, "PUT", path)).body, {
        ids: upIds,
      });
      await settle(s);
      ...
      assert.match(
        await aside.getByRole("alert").innerText(),
        new RegExp(
          `${kind === "group" ? "Group" : "Project"} order wasn't saved`,
        ),
      );
      assert.deepEqual(
        await readOrder(),
        original,
        "Failed write rolls visible order back",
      );
    }
    ...
    assert.deepEqual(
      await projectOrder(),
      ["Two", "One"],
      "Canonical order reloads from API",
    );
    ...
    assert.equal(
      api.calls.some(
        (c) => c.method === "PUT" && /^\/projects\/\d+$/.test(c.path),
      ),
      false,
      "Order must not mutate project records",
    );
  });
  ...
  await test("Settings identity and theme share live shell state", async () => {
    ...
    await attr(
      aside.getByRole("button", {
        name: "Choose theme, current: dark",
        exact: true,
      }),
      "title",
      "Theme: dark",
    ...
    assert.equal(
      await page.evaluate(() => localStorage.getItem("lific_theme")),
      "light",
    );
    assert.ok(
      (
        await page
          ...
          .getAttribute("class")
      )?.includes("bg-[var(--surface)]"),
    );
    await page.reload();
    await identity.filter({ hasText: "Updated Operator" }).waitFor();
    await aside
      .getByRole("button", {
        name: "Choose theme, current: light",
        exact: true,
      })
      .waitFor();
  });
  ...
  await test("real Layout mobile menus history and palette suppression", async () => {
    ...
    const depth = (n: number) =>
      page.waitForFunction(
        (n) => history.state?.lificMobileNav?.depth === n,
        n,
      );
    ...
    const closed = () =>
      page.waitForFunction(() => !(window as any).fixture.mobile());
    await open();
    assert.equal(
      await page.locator("main").evaluate((el) => !!el.closest("[inert]")),
      true,
    );
    for (const key of ["Control+k", "Control+p", "Meta+k", "Meta+p"])
      await page.keyboard.press(key);
    assert.equal(
      await page.evaluate(() => (window as any).fixture.palette()),
      false,
    );
    await rootPane
      .getByRole("button", { name: "Open One navigation", exact: true })
      .click();
    await depth(2);
    await page.goBack();
    await depth(1);
    ...
    await page.waitForURL("**/#/ONE/board");
    await closed();
    await page.goBack();
    await depth(0);
    assert.equal(new URL(page.url()).hash, "#/");
    await open();
    await rootPane
      .getByRole("button", { name: "Actions for One", exact: true })
      .click();
    await menuItem(page, "Move down").click();
    assert.deepEqual(
      (await called(s.api, 1, "PUT", "/projects/reorder")).body,
      { ids: [2, 1, 3, 4] },
    );
    await rootPane
    ...
    await depth(1);
    await rootPane
      .getByRole("button", { name: "New project or group", exact: true })
      .click();
    ...
    await page.waitForFunction(
      () =>
        new DOMMatrix(
          getComputedStyle(document.querySelector("[data-mobile-nav]")!)
            .transform,
        ).m41 === 0,
    );
  ...
  console.log(
    `Sidebar: ${passed} passed, ${failures.length} failed. Screenshots: ${shotDir}`,
  );
