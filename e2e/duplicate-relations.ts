...
export async function checkDuplicateRelations(
  ...
  try {
    ...
    const [source, target] = await page.evaluate(async () => {
      ...
      const project = projects.find(
        (p: { identifier: string }) => p.identifier === "DEMO",
      );
      const source = await request("/issues", {
        project_id: project.id,
        title: "Duplicate source",
      });
      const target = await request("/issues", {
        project_id: project.id,
        title: "Canonical target",
      });
      await request("/issues/link", {
    ...
    const forward = page
      .locator(".issue-meta-field")
      .filter({ hasText: "Duplicate of" });
    await forward
    ...
    await reverse
      ...
      .waitFor({ state: "visible" });
    console.log(
      "ok   duplicate-only relations render both directions on desktop and mobile",
    );
  } finally {
    await page.close();
  }
