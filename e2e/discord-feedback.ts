...
export async function checkDiscordFeedback(
  ...
  for (const width of [1440, 390]) {
    ...
    try {
      ...
      const selected = page.locator(
        '[data-issue-index] [role="checkbox"][aria-checked="true"]',
      );
      const all = page.getByRole("button", {
        name: /^(Select all|All selected)\s/,
      });
      const clear = page.getByRole("button", {
        name: "Clear selection  ·  Esc",
        exact: true,
      });
      ...
      async function assertSelection(
        ...
        assert.deepEqual(
          await selected.evaluateAll((els) =>
        ...
        if (identifiers.length) {
          ...
          assert.equal(
            await selectionCount.innerText(),
            `${identifiers.length} selected`,
          );
        ...
        if (width >= 640) {
          ...
          assert.equal(
            await all.getAttribute("aria-pressed"),
            String(complete),
          );
          const count =
            identifiers.length && !complete
              ? `${identifiers.length}/${visibleCount}`
              : visibleCount;
          assert.equal(
            (await all.innerText()).replace(/\s+/g, " ").trim(),
          ...
          assert.equal(
            await all.count(),
            0,
            "desktop select-all must not be exposed on mobile",
          );
        }
      }
      ...
      await page
        .getByRole("checkbox", { name: "Select DEMO-2", exact: true })
        .click();
      await assertSelection(["DEMO-1", "DEMO-2"], 3);
      await selectVisible();
      await assertSelection(["DEMO-1", "DEMO-2", "DEMO-3"], 3);
      await page
        .getByRole("checkbox", { name: "Select DEMO-3", exact: true })
        .click();
      await assertSelection(["DEMO-1", "DEMO-2"], 3);
      ...
      assert(
        text.includes("Smoke issue") &&
          text.includes("Second smoke issue") &&
          text.includes("First smoke comment"),
      );
      assert(!text.includes("Excluded smoke issue"));
      await page
        .getByRole("button", { name: "Export", exact: true })
        .waitFor({ state: "visible" });
      await page.route("**/api/export/issues/*", (route) =>
      ...
      await page
        .getByRole("alert")
        .filter({ hasText: "Could not export" })
        .waitFor();
      ...
      await page.waitForFunction(
        () => document.querySelectorAll("[data-issue-index]").length === 1,
      );
      ...
      assert.equal(
        ...
        0,
        "Ctrl+A in search must select text, not rows",
      );
      ...
      assert(
        !(await page.locator("body").innerText()).includes("Lucide:Terminal"),
        "malformed icon leaked text",
      );
      if (process.env.LIFIC_SCREENSHOT_DIR)
        await page.screenshot({
          path: join(
            process.env.LIFIC_SCREENSHOT_DIR,
            `lific-discord-list-${width}.png`,
          ),
        });
      await page
        .getByRole("button", { name: "Smoke issue", exact: true })
        .click();
  ...
  try {
    ...
    for (const id of ["codex-laptop", "codex-desktop", "custom-agent"]) {
      await page
        .getByRole("button", { name: "Add custom or named connection" })
        .click();
      await page
        .getByLabel("Config template")
        .selectOption(id.startsWith("codex") ? "codex" : "");
