  ...
  plugins: [
    {
      ...
      enforce: "pre",
      resolveId(id) {
        if (id === "/src/ContextMenuFixture.svelte") return fixtureId;
      },
      load(id) {
        ...
        server.middlewares.use(async (req, res, next) => {
          ...
          res.end(
            await server.transformIndexHtml(
              "/",
              `<div id="app"></div><script type="module">import {mount} from 'svelte'; import Fixture from '/src/ContextMenuFixture.svelte'; import '/src/app.css'; mount(Fixture,{target:document.getElementById('app')});</script>`,
            ),
          );
        });
      },
...
const deadline = setTimeout(() => {
  console.error("Context menu test deadline exceeded");
  process.exit(1);
}, 90_000);
let browser;
try {
  ...
  const focused = async (name: string) => {
    ...
    assert.ok(
      (await item(name).getAttribute("class"))?.includes(
        "bg-[var(--bg-subtle)]",
      ),
    );
  };
  ...
  await focused("Middle");
  await page.keyboard.press("End");
  await focused("Native link");
  await page.keyboard.press("ArrowDown");
  await focused("First");
  ...
  await focused("First");
  await item("Middle").hover();
  await focused("Middle");
  await item("Disabled link").hover();
  await focused("Middle");
  await page.keyboard.press("Enter");
  await closedAt("trigger");
  assert.deepEqual(await page.evaluate(() => (window as any).actions), [
    "middle",
  ]);
  ...
  await page.evaluate(() => {
    ...
    window.addEventListener(
      "keydown",
      (e) => {
        if (e.key === "Escape") (window as any).escapes++;
      },
      true,
    );
  ...
  await page.locator("#right-target").click({ button: "right" });
  await focused("First");
  await page.keyboard.press("Escape");
  await closedAt("right-target");
  assert.equal(
    await page.locator("#right-target").getAttribute("tabindex"),
    "-1",
  );
  await page.locator("#editor").focus();
  assert.equal(
  ...
  await open();
  await item("Focus editor").click();
  await closedAt("editor");
  await page.evaluate(
    () =>
      new Promise((resolve) =>
  ...
  for (const mode of ["disabled", "empty"]) {
    ...
    await page.keyboard.press("Enter");
    assert.equal(
      await page.evaluate(() => (window as any).fixture.menuOpen()),
      true,
    );
    await page.keyboard.press("Escape");
    await closedAt("trigger");
  ...
  await open();
  await item("Native link").click();
  await closedAt("trigger");
  assert.ok(page.url().endsWith("#/native"));
  await open();
  await item("Navigate").focus();
  ...
  assert.equal(
    await page.evaluate(() => (window as any).fixture.suppressed()),
    true,
  );
  for (const key of ["Control+k", "Control+p", "Meta+k", "Meta+p"])
    await page.keyboard.press(key);
  assert.equal(
    await page.evaluate(() => (window as any).fixture.paletteOpen()),
    false,
  );
  ...
  await page.waitForFunction(
    () => document.querySelector("#theme")?.textContent === "dark:dark",
  );
  assert.equal(
  ...
  await page.waitForFunction(
    () => document.querySelector("#theme")?.textContent === "system:dark",
  );
  await page.emulateMedia({ colorScheme: "light" });
  await page.waitForFunction(
    () => document.querySelector("#theme")?.textContent === "system:light",
  );
    ...
    const data =
      url.pathname === "/api/auth/me"
        ? route.request().method() === "PATCH"
          ? { ...user, ...route.request().postDataJSON() }
          : user
        : url.pathname === "/api/instance"
  ...
  await page.waitForFunction(
    () => document.querySelector("#identity")?.textContent === "Before",
  );
  await page.getByLabel("Display name", { exact: true }).fill("After");
  await page.getByRole("button", { name: "Save changes", exact: true }).click();
  await page.waitForFunction(
    () => document.querySelector("#identity")?.textContent === "After",
  );
  await page.evaluate(() => (window as any).fixture.setTheme("dark"));
  await page.waitForFunction(
    () => document.querySelector("#theme")?.textContent === "dark:dark",
  ...
  console.log(
    "Context menu: focus, disabled items, arrows/Home/End, hover/Enter, Tab, capture Escape, right-click, action focus, links, mobile guards; reactive theme/storage and Settings profile passed.",
  );
} finally {
  await browser?.close();
  await server.close();
