...
- **An API key can no longer mint another API key or connect a tool.** Creating a durable credential now requires a browser session that authenticated in the last 15 minutes. Previously a leaked key was a key factory: an attacker could mint a spare that outlived the revocation of the key they came in on. OAuth tokens were already refused on those endpoints. Web UI behavior is unchanged, and `lific connect` is unaffected.
...
- **Six endpoints now require a browser session created within the last 15 minutes** and answer `403 recent authentication required` to an API key or OAuth token: `POST /api/auth/keys`, `POST /api/auth/bots`, `POST /api/users`, `POST /api/users/{id}/promote`, `POST /api/users/{id}/reactivate`, `PATCH /api/instance/settings`, plus `POST /api/projects/{id}/members` and role *increases* through `PATCH /api/projects/{id}/members/{user_id}`. Scripted provisioning moves to `lific user create`, `lific user promote` and `lific key create` on the server. `demote`, `deactivate`, role downgrades and member removal are unchanged.
